...
local function GetAnimName(inst, basename)
	return basename..(IsSwingRight(inst) and "right" or "")..(IsOpen(inst) and "_open" or "")
end
...
local function GetDoorRotationOffset(inst, rot)
	...
	return len * (math.sin(angle2) - math.sin(angle1)),
		   0,
...
local function MakeWall(name, builds, isdoor)
	...
	if isdoor then
		...
		assets ={Asset("ANIM", "anim/"..builds.wide..".zip"),}
	...
	local function CanActivate(inst, doer)
		return doer.components.inventory:Has(name.."_item",1)
	end

	local function Activate(inst, doer)
		--[[         local truestruct = SpawnPrefab(name)          doer.components.inventory:ConsumeByName(name.."_item",1)         truestruct.Transform:SetPosition(inst.Transform:GetWorldPosition())         truestruct.Transform:SetRotation(inst.Transform:GetRotation())         ]]
		local wall = SpawnPrefab(name)
		local rot = inst.Transform:GetRotation()
		local x, y, z = inst.Transform:GetWorldPosition()
		if wall ~= nil then
       	...
       	doer.components.inventory:ConsumeByName(name.."_item",1)
		onhammered(inst)
	end
	...
	local function fn()
		...
		if isdoor then
			...
			inst.dooranim.AnimState:SetMultColour(TUNING.VIRTUALRED,TUNING.VIRTUALGREEN,TUNING.VIRTUALBLUE,TUNING.VIRTUALALPHA)
	...
	return Prefab(VIRTUAL_PREFIX..name, fn, assets, custom_wall_prefabs or wall_prefabs)
end
...
local function MakeInvItem(name, placement, animdata, isdoor)
	local assets ={Asset("ANIM", "anim/"..animdata..".zip"),}
	local item_prefabs =     {         placement,     }
	local function ondeploywall(inst, pt, deployer, rot)
		local wall = SpawnPrefab(VIRTUAL_PREFIX..placement)
		if wall ~= nil then
	...
	return Prefab(VIRTUAL_PREFIX..name, itemfn, assets, item_prefabs)
end
...
local function MakeWallPlacer(placer, placement, builds, isdoor)
	...
	local CreateDoorAnim = isdoor and notdoor


	return MakePlacer(placer,builds.wide,builds.wide,not isdoor and "idle" or nil,nil, nil, true, nil, 0, "eight",
		function(inst)
...
return MakeWall("fence", {wide="fence", narrow="fence_thin"}, false),
     ...
     MakeWallPlacer(VIRTUAL_PREFIX.."fence_item_placer", "fence", {wide="fence", narrow="fence_thin"}, false),
     MakeWall("fence_gate", {wide="fence_gate", narrow="fence_gate_thin"}, false),
     MakeInvItem("fence_gate_item", "fence_gate", "fence_gate", false),
     MakeWallPlacer(VIRTUAL_PREFIX.."fence_gate_item_placer", "fence_gate", {wide="fence_gate", narrow="fence_gate_thin"}, false)
